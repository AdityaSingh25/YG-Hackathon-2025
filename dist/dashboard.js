...
function serveDashboard(node, httpPort) {
    ...
    app.get("/debug/ping", async (req, res) => {
        try {
            ...
            const t0 = Date.now();
            const r = await (0, protocol_1.rpc)(host, port, { type: "ping" });
            if (r && r.type === "pong")
    ...
    app.post("/upload", upload.single("file"), async (req, res) => {
        try {
            ...
            if (encrypt) {
                ...
                link = (0, client_1.makeShareLink)(hash, name, key, iv);
            }
            else {
                hash = await node.storeLocalAndReplicate(data, meta);
                link = (0, client_1.makeShareLink)(hash, name);
            }
    ...
    app.get("/download/:hash", async (req, res) => {
        try {
            ...
            if (!buf) {
                ...
                if (!peers.length)
                    return res.status(404).send("not found (no peers)");
                buf = await (0, client_1.fetchBytes)(peers, h);
    ...
    app.get("/bylink", async (req, res) => {
        try {
            ...
            const { cid, key, iv, name } = (0, client_1.parseShareLink)(link);
            const peers = node.getPeersList();
            let buf = await (0, client_1.fetchBytes)(peers, cid);
            if (key && iv) {
                const { decryptAESGCM } = await Promise.resolve().then(() => __importStar(require("./crypto")));
